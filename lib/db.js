...
const actionOneOrMany = curry((action, collection, isMultiple, ...params) => {
  const type = isMultiple ? `${action}Many` : `${action}One`;
  debug('db.' + collection.collectionName + '.' + type, ...params);
  return new Promise((resolve, reject) => {
    collection[type](...params, (err, r) => {
      if (err) return reject(err);
      return resolve(normalizeResult(r));
    });
  });
});
...
class Database extends EventEmitter {
  ...
  async init(uri) {
    ...
    this._db.collection = promisify(this._db.collection);
    return this;
  }
