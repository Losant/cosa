...
class Database extends EventEmitter {
  ...
  async insert(collectionName, docs, options) {
    ...
    const collection = await this.collection(collectionName);
    return insertOneOrMany(collection, Array.isArray(docs), docs, options);
  }
  ...
  async update(collectionName, query, update, options) {
    ...
    const collection = await this.collection(collectionName);
    const updateOptions = { upsert: options.upsert, writeConcern: options.writeConcern };
  ...
  async remove(collectionName, query, options) {
    ...
    query = EJSON.deserialize(query);
    const collection = await this.collection(collectionName);
    const removeOpts = { writeConcern: options.writeConcern };
    return removeOneOrMany(collection, options.multiple, query, removeOpts);
  }
  ...
  async replace(collectionName, query, replace, options) {
    ...
    const r = await collection.replaceOne(query, replace, options);
    return normalizeResult(r);
  }
