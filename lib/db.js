  ...
  async find(collectionName, query, options) {
    ...
    if (options.count) {
      ...
      if ((!query || isEmpty(query))) {
        let count = collection.estimatedDocumentCount(options);
        if (countOptions.skip) {
          count = count - options.skip;
          if (count < 0) {
            return 0;
          }
        }
        if (countOptions.limit && countOptions.limit <= count) {
          return countOptions.limit;
        }
        return count;
      } else {
