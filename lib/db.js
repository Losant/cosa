...
class Database extends EventEmitter {
   ...
   * @param {object} [options.batchSize] - number of items per batch (default in mongo driver is 1000)
   * @param {object} [options.noCursorTimeout] - boolan, if the cursor can time out after being idle, mongo driver default is false
   ...
   * @param {object} [options.writeConcern] - the write concern options https://mongodb.github.io/node-mongodb-native/4.0/classes/writeconcern.html
  ...
  async insert(collectionName, docs, options = {}) {
    ...
    const collection = await this.collection(collectionName);
    const results = await insertOneOrMany(collection, Array.isArray(docs), docs, { writeConcern: options.writeConcern });
  ...
  async distinct(collectionName, key, query, options = {}) {
    ...
    query = deserialize(query);
    const collection = await this.collection(collectionName);
    return collection.distinct(key, query, { readPreference: options.readPreference });
  }
  ...
  async replace(collectionName, query, replace, options = {}) {
    ...
    const r = await collection.replaceOne(query, replace, { writeConcern: options.writeConcern });
    r.ops = replace;
    return normalizeResult(r);
  }
