...
class Database extends EventEmitter {
  ...
  //  */
  // startSession(opts) {
  //   return this._client.startSession(opts);
  // }
  ...
  async insert(collectionName, docs, { writeConcern, session } = {}) {
    ...
    const collection = await this.collection(collectionName);
    const results = await insertOneOrMany(collection, Array.isArray(docs), docs, { writeConcern, session: session?.get() });
  ...
  async replace(collectionName, query, replace, { writeConcern, session } = {}) {
    ...
    const r = await collection.replaceOne(query, replace, { writeConcern, session: session?.get() });
    r.ops = replace;
    return normalizeResult(r);
  }
