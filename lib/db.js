...
class Database extends EventEmitter {
  ...
  async insert(collectionName, docs, { writeConcern, session } = {}) {
    ...
    const collection = await this.collection(collectionName);
    const results = await insertOneOrMany(collection, Array.isArray(docs), docs, { writeConcern, session });
  ...
  async replace(collectionName, query, replace, { writeConcern, session } = {}) {
    ...
    const r = await collection.replaceOne(query, replace, { writeConcern, session });
    r.ops = replace;
    return normalizeResult(r);
  }
