...
const _create = (data, definition) => {
  ...
  const _saveHelper = async function(context, options = {}, explicitId) {
    ...
    if ('function' === typeof newOrUpdatedModel.afterSaveCommit) {
      if (options.session) {
        options.session.afterCommits.push(newOrUpdatedModel.afterSaveCommit.bind(newOrUpdatedModel));
      } else {
        const afterCommit = newOrUpdatedModel.afterSaveCommit(original || null);
