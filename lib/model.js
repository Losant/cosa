...
const _create = (data, definition) => {
  ...
  const _saveHelper = async function(context, options) {
    ...
    if (!context.isNew()) {
      ...
      obj._etag = newEtag;
      if (options.canPassID) { obj._id = pathOr(obj._id, ['_id'], options); }
  ...
  definition.methods.saveWithId = async function(options) {
    options = Object.assign(options || {}, { canPassID: true });
    return _saveHelper(this, options);
  };
