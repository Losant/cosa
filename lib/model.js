...
const { pathEq, complement } = require('omnibelt');
...
const _create = (data, definition) => {
  ...
  definition.methods._validate = function(obj, options) {
    ...
    return joiValidate(obj, definition._schema, options);
  };
  ...
  definition.methods.save = async function(options) {
    ...
    if (!this.isNew()) {
      ...
      console.log(obj._id instanceof ObjectID);
      // console.log('IS INSTANCE OF OBJECT ID AFTER INSERT ', (obj._id instanceof ObjectID));
      newOrUpdatedModel = _create(obj, definition);
      // await this._validate(newOrUpdatedModel, options);
    }
    if ('function' === typeof newOrUpdatedModel.afterSave) {
      // should not be awaited
      newOrUpdatedModel.afterSave(original || null);
    }
    return newOrUpdatedModel;
  };

  definition.methods.remove = async function() {
    const collection = definition.collection;
