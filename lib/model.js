  ...
  const _saveHelper = async function(context, options = {}, explicitId) {
    ...
    if (options.session) {
      if ('function' === typeof newOrUpdatedModel.afterSave) {
        await newOrUpdatedModel.afterSave(original || null, options);
      }
      const boundArgs = [ newOrUpdatedModel, original || null, options ];
      if ('function' === typeof newOrUpdatedModel.afterSaveCommit) {
        options.session.afterCommits.push(newOrUpdatedModel.afterSaveCommit.bind(...boundArgs));
      }

      if (newOrUpdatedModel.afterSaveAbort) {
        options.session.onAborts.push(newOrUpdatedModel.afterSaveAbort.bind(...boundArgs));
      }
    } else {
      let chain = Promise.resolve();
      if ('function' === typeof newOrUpdatedModel.afterSave) {
        chain = chain.then(() => {
          return newOrUpdatedModel.afterSave(original || null, options);
        });
      }
      if ('function' === typeof newOrUpdatedModel.afterSaveCommit) {
        chain = chain.then(() => {
          return newOrUpdatedModel.afterSaveCommit(original || null, options);
        });
      }
