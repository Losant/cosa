...
const isNotVirtual = pathEq([ 'type' ], 'virtual');
  ...
  definition.methods.save = async function(options) {
    ...
    const before = async () => {
      if ('function' === typeof this.beforeSave) {
        return this.beforeSave.apply(obj);
      }
    };
    await before();
    ...
    if (!this.isNew()) {
      ...
      const result = await db.update(collection, query, obj);
