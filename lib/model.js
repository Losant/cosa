    ...
    find: function (query, options) {
      ...
      return db.find(collection, query, options)
        .then(function (dbCursor) {
          var cursor = new Cursor(dbCursor, function (item) {
...
function _serializeArray (arr) {
  console.log('_serializeArray: %j', arr);
  return arr.map(_serialize.bind(null));
}

function _serializePrimitive (val) {
  return val;
}
...
function _serializeObject (obj) {
  if (obj && 'function' === typeof obj.toJSON) {
    ...
    for (var p in obj) {
      if (obj[p] && 'function' === typeof obj[p].toJSON) {
        obj[p] = obj[p].toJSON();
      }
    }
    return obj;
...
function _serialize (val) {
  if (Array.isArray(val)) {
    ...
    return _serializePrimitive(val);
  }
