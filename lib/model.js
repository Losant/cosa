...
const _create = (data, definition) => {
  ...
  const _saveHelper = async function(context, options = {}, explicitId) {
    ...
    if ('function' === typeof context.beforeSave) {
      const beforeCommitFunc = await context.beforeSave.apply(obj, [options]);
      if (beforeCommitFunc) {
        if (options.session) {
          options.session.beforeCommits.push(beforeCommitFunc);
        } else {
          await beforeCommitFunc();
        }
      }
    }
    ...
    if ('function' === typeof newOrUpdatedModel.afterSave) {
      if (options.session) {
        const afterSave = await newOrUpdatedModel.afterSave(original || null, options);
        if (typeof afterSave === 'function') {
          options.session.afterCommits.push(afterSave.bind(newOrUpdatedModel));
        }
      } else {
        // should not be awaited
        let afterSave = newOrUpdatedModel.afterSave(original || null, options);
        if (typeof afterSave === 'function') {
          afterSave = afterSave.apply(newOrUpdatedModel);
        }
        if (options.waitAfterSave) { await afterSave; }
      }
    }
    if (options.session && newOrUpdatedModel.onAbortTransaction) {
      options.session.onAborts.push(newOrUpdatedModel.onAbortTransaction.bind(newOrUpdatedModel));
    }
    if (options.createSession) {
