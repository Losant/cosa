...
const _create = (data, definition, applyGlobalSession) => {
  ...
  const _saveHelper = async function(context, options = {}, explicitId) {
    ...
    if ('function' === typeof context.beforeSave) {
      await context.beforeSave.apply(obj, [applyGlobalSession(options)]);
    }
    ...
    if (!context.isNew()) {
      ...
      try {
        result = await db.replace(collection, query, obj, applyGlobalSession(options));
      } catch (e) {
        let error = e;
      ...
      try {
        result = await db.insert(collection, obj, applyGlobalSession(options));
      } catch (e) {
        let error = e;
        if ((error.code === 11000 || error.message.startsWith('E11000 duplicate key')) && context.transformDuplicateKeyError) {
          error = context.transformDuplicateKeyError(error);
      ...
      newOrUpdatedModel = _create(obj, definition, applyGlobalSession);
    }
    if ('function' === typeof newOrUpdatedModel.afterSave) {
      if (definition.session) {
        // await definition.session.withTransaction(async () => {
        //   return newOrUpdatedModel.afterSave(original || null, applyGlobalSession(options));
        // });
        definition.session.on('committed', () => {
          console.log('on committed...');
          console.log(newOrUpdatedModel.toObject());
        });
      } else {
      // should not be awaited
        const afterSave = newOrUpdatedModel.afterSave(original || null, applyGlobalSession(options));
  ...
  definition.methods.reload = async function(options = {}) {
    ...
    return _create(result, definition, applyGlobalSession);
  };
...
const define = (definition) => {
  ...
  const applyGlobalSession = (options) => {
    if (definition.session) {
      options.session = definition.session;
    }
    return options;
  };
  ...
  return {
    ...
    create: (data) => {
      return _create(data, definition, applyGlobalSession);
    },
    ...
    useSession: (session) => {
      return _useSession(session, definition);
    },
    ...
    count: (query, options = {}) => {
      ...
      return db.find(collection, query, applyGlobalSession({ ...options, count: true }));
    },
    ...
    find: async (query, options = {}) => {
      ...
      const dbCursor = await db.find(collection, query, applyGlobalSession(options));
      const cursor = new Cursor(dbCursor, (item) => {
        return _create(item, definition, applyGlobalSession);
      });
    ...
    findOne: async (query, options = {}) => {
      ...
      const result = await db.find(collection, query, applyGlobalSession({ ...options, findOne: true }));
      return !result ? null : _create(result, definition, applyGlobalSession);
    },
    ...
    update: (query, update, { autoSet = true, ...options } = {}) => {
      ...
      return db.update(collection, query, update, applyGlobalSession(options));
    },
    ...
    remove: (query, options = {}) => {
      ...
      return db.remove(collection, query, applyGlobalSession(options));
    },
    ...
    distinct: (key, query, options = {}) => {
      ...
      return db.distinct(collection, key, query, applyGlobalSession(options));
    },

    aggregate: (pipeline, options = {}) => {
      const collection = definition.collection;
      debug(`aggregating ${JSON.stringify(pipeline)} from ${collection}`);
      return db.aggregate(collection, pipeline, applyGlobalSession(options));
    },
    ...
    project: async (query, value, options = {}) => {
      ...
      let dbCursor = await db.find(collection, query, applyGlobalSession(options));
...
_useSession = (session, definition) => {
  const newDef = clone(definition);
  newDef.session = session;
